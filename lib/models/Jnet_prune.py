...
def conv_dw_wo_bn_gp(inp, oup, kernel_size, stride, padding=1, gp=1):
    return nn.Sequential(
        nn.Conv2d(inp, inp, kernel_size, stride, padding=padding, groups=gp, bias=False),
        nn.BatchNorm2d(inp),
        nn.ReLU(inplace=True),
        nn.Conv2d(inp, oup, 1, 1, 0, bias=False)
    )
...
class BasicBlock3x3(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, dilation=1, gp=True):
        ...
        if gp:
            self.conv1 = Conv(in_planes, out_planes, kernel_size=3, stride=stride, bias=False, gp=gp, dilation=dilation, bn=False, relu=False)
        else:
...
# channel, depth 成比例的降低
# upsample 性能也不差

class PoseNet(nn.Module):
    # (2, 3, 4, 1); (1,2,3,1)
    def __init__(self, block=Bottleneck, layers=(1, 2, 3, 1), nstack=1, inp_dim=256, oup_dim=17, num_parts=17, slim_cfg=None):
        super(PoseNet, self).__init__()
        base_channels = 32
        # slimming
        if slim_cfg == None:
            slim_cfg = [[base_channels], [base_channels], [base_channels * 2], [base_channels * 4], [base_channels * 2, base_channels * 2]]
            slim_cfg = [i for sub_i in slim_cfg for i in sub_i]
        self.num_parts = num_parts

        self.inplanes = base_channels
        self.conv1 = nn.Conv2d(3, slim_cfg[0], kernel_size=5, stride=2, padding=2, bias=False)
        # self.conv1 = conv_dw_wo_bn_gp(3, base_channels, 5, 2, 2, gp=3)
        self.bn1 = torch.nn.BatchNorm2d(slim_cfg[0])
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.avgpool = nn.AvgPool2d(7, stride=1)
        self.layer1 = self._make_layer(block, slim_cfg[1], layers[0])
        self.layer2 = self._make_layer(block, slim_cfg[2], layers[1])
        self.layer3 = self._make_layer(block, slim_cfg[3], layers[2])

        # deconv params
        self.num_deconv_layers = 2
        self.num_deconv_filters = [slim_cfg[4], slim_cfg[5]]
...
def get_pose_net(cfg, args, is_train, slim_cfg=None, **kwargs):
    model = PoseNet(slim_cfg=slim_cfg)
