...
class Conv(nn.Module):
    def __init__(self, inp_dim, out_dim, kernel_size=3, stride=1, bn=False, relu=True, gp=False, shuffle=False, bias=False):
...
class BasicBlock3x3(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, dilation=1, gp=True):
        ...
        if gp:
            self.conv1 = Conv(in_planes, out_planes, kernel_size=3, stride=stride, bias=False)
        else:
            self.conv1 = nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False)
...
class PoseNet(nn.Module):
    def __init__(self, block=Bottleneck, layers=(2, 3, 4, 1), nstack=1, inp_dim=256, oup_dim=17, num_parts=17):
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels):
        ...
        for i in range(num_layers):
            ...
            layers.append(
                nn.ConvTranspose2d(in_channels=self.last_channel, out_channels=planes, kernel_size=kernel, stride=2,
