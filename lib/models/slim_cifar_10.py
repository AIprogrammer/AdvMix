...
def main():
    ...
    if args.retrain:
        ...
        model.load_state_dict(torch.load('vgg19_cifar10.pth', torch.device('cpu')))
    ...
    new_model.to(device)
    new_model.load_state_dict(torch.load('pruned_vgg19_cifar10.pth', torch.device('cpu')))
    # test(new_model, device, test_loader)
    # top1 = 93.74%

    # speed up
    print('==> old model', new_model)
    dummy_input = torch.randn(2, 3, 32, 32) 
    m_speedup = ModelSpeedup(new_model, dummy_input, 'mask_vgg19_cifar10.pth', torch.device('cpu')) # 根据mask得到pruned model
    m_speedup.speedup_model()
    dummy_input = dummy_input
    print('==> speed up', new_model)
    out = model(dummy_input)

    print('==>', out)
    
# /home/sensetime/anaconda3/envs/torch_1_3/lib/python3.6/site-packages/nni/compression/speedup/torch/compressor.py
