...
class Conv(nn.Module):
    def __init__(self, inp_dim, out_dim, kernel_size=3, stride=1, bn=False, relu=True, gp=False, shuffle=True, spade=False, config_text='spadesyncbatch3x3'):
        ...
        self.spade = spade
        if spade:
            self.bn = SPADE(config_text, out_dim, LABEL_NC)
        else:
            if bn:
    ...
    def forward(self, x, seg=None):
        ...
        x = self.conv(x)
        if self.spade and seg is not None:
            x = self.bn(x, seg)
        else:
...
class light_model(nn.Module):
    def __init__(self, settings):
        ...
        if self.upsample4x:
            ...
            self.spade_block = Conv(64,64,3,1,True, True, spade=True)
    ...
    def forward(self, x: Variable):
        ...
        if self.upsample4x:
            ...
            innercat_s2 = self.spade_block(innercat_s2, seg) # 64, 3
            if self.crosscat:
