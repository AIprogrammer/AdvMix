...
class light_model(nn.Module):
    def __init__(self, cfg, args, settings):
        ...
        if self.settings['sepa']:
            ...
            self.pre_half_1 = Conv(18, 18, 3, 1, bn=True, gp=settings['pre2'][0], shuffle=settings['pre2'][1])
            self.pre_half_2 = Conv(18, 18, 3, 1, bn=True, gp=settings['pre3'][0], shuffle=settings['pre3'][1])
    ...
    def forward(self, x: Variable):
        if self.settings['sepa']:
            x = self.pre(x)
            x_1 = self.pre_half_1(x[:,:18,...])
            x_2 = self.pre_half_1(x[:,18:36,...])
        ...
        if self.upsample4x:
            ...
            if self.settings['up4x_sepa']:
                ...
                mid4x_1 = self.stage2_pre_up4x_1(mid4x[:,:36,...])
                mid4x_2 = self.stage2_pre_up4x_2(mid4x[:,36:,...])
...
def get_pose_net(cfg, args, is_train, **kwargs):
    ...
    'remember to change size in .yaml'
