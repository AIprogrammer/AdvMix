...
class COCODataset(JointsDataset):
    ...
    def _load_image_set_index(self):
        """ image id: int """
        if self.mini_coco:
            image_ids = self.coco.getImgIds()[:200]
        else:
    ...
    def _load_coco_keypoint_annotation_kernal(self, index):
        ...
        rec = []
        for kobj, obj in enumerate(objs):
            ...
            rec.append({
                ...
                'filename': '',
                'imgnum': 0,
                'instance_index':str(index) + '_' + str(kobj)
            })
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, img_path,
        ...
        if self.test_robust and self.corruption_type != 'clean':
            ...
            res_file = os.path.join(
                res_folder, 'keypoints_{}_results_{}_{}_{}.json'.format(
                    self.image_set, rank, self.corruption_type, self.severity)
            )
        ...
        for idx, kpt in enumerate(preds):
            image_idx = int(img_path[idx][-16:-4])
            _kpts.append({
                'keypoints': kpt,
